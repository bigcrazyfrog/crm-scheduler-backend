...
def stop_containers(context, containers):
    ...
    cmd = f"docker-compose -f docker-compose-dev.yml stop {' '.join(containers)}"
    context.run(cmd)
...
def clear(context):
    ...
    context.run("docker compose -f docker-compose-dev.yml rm -f")
    context.run("docker compose -f docker-compose-dev.yml down -v --rmi all --remove-orphans")
